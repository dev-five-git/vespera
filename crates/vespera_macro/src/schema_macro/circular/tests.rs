...
fn test_has_fk_relations_unnamed_fields() {
    assert!(!analyze_circular_refs(&[], "pub struct TupleStruct(i32, String);").has_fk_relations);
}
...
fn test_generate_default_for_relation_field_has_many() {
    ...
    assert!(
        generate_default_for_relation_field(&ty, &ident("users"), &[], &fields("{ pub id: i32 }"))
            .to_string()
            .contains("users : vec ! []")
    );
}
...
fn test_generate_default_for_relation_field_unknown_type() {
    ...
    assert!(
        generate_default_for_relation_field(&ty, &ident("field"), &[], &fields("{ pub id: i32 }"))
            .to_string()
...
fn test_generate_default_for_relation_field_has_one_no_fk_found() {
    let ty: syn::Type = syn::parse_str("HasOne<user::Entity>").unwrap();
    assert!(
        generate_default_for_relation_field(&ty, &ident("user"), &[], &fields("{ pub id: i32 }"))
            .to_string()
            .contains("user : None")
    );
}
