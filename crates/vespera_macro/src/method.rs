...
mod tests {
    ...
    fn test_http_method_to_token_stream(#[case] method: HttpMethod, #[case] expected_method_name: &str) {
        ...
        assert!(code.contains(expected_method_name), "Code should contain method name: {}, got: {}", expected_method_name, code);

        // Check that it contains the routing path
        assert!(code.contains("routing"), "Code should contain 'routing', got: {}", code);

        // Check that it contains the axum path
        assert!(code.contains("axum"), "Code should contain 'axum', got: {}", code);

        // Check that it contains the vespera path
        assert!(code.contains("vespera"), "Code should contain 'vespera', got: {}", code);
    }

    #[test]
    fn test_http_method_to_token_stream_all_methods() {
        // Test that all methods generate valid TokenStreams
        let methods = vec![HttpMethod::Get, HttpMethod::Post, HttpMethod::Put, HttpMethod::Patch, HttpMethod::Delete, HttpMethod::Head, HttpMethod::Options, HttpMethod::Trace];
        ...
        for method in methods {
            ...
            assert!(!code.is_empty(), "TokenStream should not be empty for {:?}", method);
            assert!(code.contains("routing"), "TokenStream should contain 'routing' for {:?}, got: {}", method, code);
        }
    }
}
