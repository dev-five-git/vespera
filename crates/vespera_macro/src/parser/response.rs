...
fn extract_result_types(ty: &Type) -> Option<(Type, Type)> {
    ...
    if is_result {
        ...
        if let Some(segment) = path.segments.last()
            && let syn::PathArguments::AngleBracketed(args) = &segment.arguments
            && args.args.len() >= 2
            && let (Some(syn::GenericArgument::Type(ok_ty)), Some(syn::GenericArgument::Type(err_ty))) = (args.args.first(), args.args.get(1))
        {
...
fn extract_status_code_tuple(err_ty: &Type) -> Option<(u16, Type)> {
        ...
        && !&type_path.path.segments.is_empty()
    {
        ...
        let is_status_code = segment.ident == "StatusCode" || (path.segments.len() > 1 && path.segments.iter().any(|s| s.ident == "StatusCode"));
...
fn extract_ok_payload_and_headers(ok_ty: &Type) -> (Type, Option<HashMap<String, Header>>) {
    if let Type::Tuple(tuple) = ok_ty {
        ...
        if let Some(payload_ty) = payload_ty {
            let headers = if has_headers { Some(HashMap::new()) } else { None };
...
pub fn parse_return_type(return_type: &ReturnType, known_schemas: &HashMap<String, String>, struct_definitions: &HashMap<String, String>) -> BTreeMap<String, Response> {
    let mut responses = BTreeMap::new();

    match return_type {
        ReturnType::Default => {
            // No return type - just 200 with no content
            responses.insert("200".to_string(), Response { description: "Successful response".to_string(), headers: None, content: None });
        }
        ReturnType::Type(_, ty) => {
            ...
            if let Some((ok_ty, err_ty)) = extract_result_types(ty) {
                ...
                let ok_schema = parse_type_to_schema_ref_with_schemas(&ok_payload_ty, known_schemas, struct_definitions);
                let mut ok_content = BTreeMap::new();
                ok_content.insert("application/json".to_string(), MediaType { schema: Some(ok_schema), example: None, examples: None });

                responses.insert("200".to_string(), Response { description: "Successful response".to_string(), headers: ok_headers, content: Some(ok_content) });
                ...
                if let Some((status_code, error_type)) = extract_status_code_tuple(&err_ty) {
                    ...
                    let err_schema = parse_type_to_schema_ref_with_schemas(&error_type, known_schemas, struct_definitions);
                    let mut err_content = BTreeMap::new();
                    err_content.insert("application/json".to_string(), MediaType { schema: Some(err_schema), example: None, examples: None });

                    responses.insert(status_code.to_string(), Response { description: "Error response".to_string(), headers: None, content: Some(err_content) });
                } else {
                    // Regular error type - use default 400
                    // Unwrap Json if present
                    let err_ty_unwrapped = unwrap_json(&err_ty);
                    let err_schema = parse_type_to_schema_ref_with_schemas(err_ty_unwrapped, known_schemas, struct_definitions);
                    let mut err_content = BTreeMap::new();
                    err_content.insert("application/json".to_string(), MediaType { schema: Some(err_schema), example: None, examples: None });

                    responses.insert("400".to_string(), Response { description: "Error response".to_string(), headers: None, content: Some(err_content) });
                }
            } else {
                // Not a Result type - regular response
                // Unwrap Json<T> if present
                let unwrapped_ty = unwrap_json(ty);
                let schema = parse_type_to_schema_ref_with_schemas(unwrapped_ty, known_schemas, struct_definitions);
                let mut content = BTreeMap::new();
                content.insert("application/json".to_string(), MediaType { schema: Some(schema), example: None, examples: None });

                responses.insert("200".to_string(), Response { description: "Successful response".to_string(), headers: None, content: Some(content) });
            }
        }
    }
...
mod tests {
    ...
    #[case("-> Result<String, (StatusCode, String)>", "Result<String, (StatusCode, String)>")] // Result with status code tuple
    ...
    fn test_parse_return_type(#[case] return_type_str: &str, #[case] expected_type: &str) {
        ...
        let return_type = if return_type_str.is_empty() {
            ...
            let parsed: syn::Signature = syn::parse_str(&full_signature).expect("Failed to parse return type");
            parsed.output
        };
